...
export function PlayPauseButton({ isPlaying, onClick, size = "md" }: { isPlaying: boolean; onClick: () => void; size?: "sm" | "md" | "lg" }) {
  ...
  const btnClass = size === "lg" ? "h-[52px] w-[52px]" : size === "sm" ? "h-10 w-10" : "h-12 w-12";
  
  return (
    <button 
      onClick={onClick}
      aria-label={isPlaying ? "Pause track" : "Play track"}
      className={`${btnClass} flex items-center justify-center rounded-full bg-gradient-to-b from-accent to-accent/90 ring-1 ring-white/30 shadow-[0_4px_16px_rgba(245,166,35,0.4)] transition-all hover:scale-105 hover:brightness-110 active:scale-95 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white text-black`}
    >
      {isPlaying ? (
        ...
        <svg className={`${iconClass} ml-1`} viewBox="0 0 24 24" fill="currentColor">
...
export function PrevNextButton({ isNext, onClick }: { isNext?: boolean; onClick: () => void }) {
  return (
    <button 
      ...
      className="min-h-[44px] min-w-[44px] flex items-center justify-center rounded-full text-white/85 transition-colors hover:text-white hover:bg-white/15 active:scale-95 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent"
    >
...
export function Seekbar({ progress, duration, onSeek }: { progress: number; duration: number; onSeek: (time: number) => void }) {
  ...
  return (
    <div 
      className="relative flex h-[24px] w-full items-center cursor-pointer touch-none group"
    ...
    >
      <div className="w-full h-[3px] bg-white/20 rounded-full overflow-hidden transition-all group-hover:h-[4px]">
        <div 
          className="h-full bg-accent relative drop-shadow-[0_0_6px_rgba(245,166,35,0.7)]"
      ...
      <div 
        className={`absolute h-3.5 w-3.5 rounded-full bg-white shadow-md transition-opacity -ml-[7px] ${isDragging ? 'opacity-100 scale-110' : 'opacity-0 group-hover:opacity-100'}`}
